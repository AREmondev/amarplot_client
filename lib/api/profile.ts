...
export const profileService = {
  ...
  updateProfile: async (token: string, updatedProfile: Partial<User> | FormData): Promise<User> => {
    const config = {
      headers: {
  ...
  leaveCommunity: async (communityId: string): Promise<any> => {
    const res = await apiClient.delete(`/user/me/joined-communities/${communityId}`);
    return res.data;
  },

  rejoinCommunity: async (communityId: string): Promise<any> => {
    const res = await apiClient.post(`/user/me/joined-communities/${communityId}`);
  ...
  removeSavedProperty: async (propertyId: string): Promise<any> => {
    const res = await apiClient.delete(`/user/me/saved-properties/${propertyId}`);
    return res.data;
  },

  getPropertiesByIds: async (propertyIds: string[]): Promise<any[]> => {
    const res = await apiClient.post('/properties/by-ids', { ids: propertyIds });
